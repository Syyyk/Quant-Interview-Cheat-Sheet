...
多重共线性会导致 $X^TX$ 某些特征值非常小,
求逆后方差膨胀(放大样本噪声), 系数不稳定, t检验失效.
衡量多重共线性 $VIF_j = \frac{1}{1-R_j^2}$, 大于10认为严重.
系数有 $SE(\beta_j) = \frac{\sigma^2 VIF_j}{(n-1)\hat{Var}(X_j)}$.
解决方法: 删除列, 做PCA, 使用Ridge/Lasso.

Ridge回归目标函数$\min_{\beta} \|y-X\beta \|^2 + \lambda \|\beta\|_2^2$,
通过惩罚项约束系数的大小.
有闭式解 $\hat{\beta} = (X^TX+\lambda I)^{-1}X^Ty$.
惩罚项避免了特征值接近0时求逆的不稳定.
Lasso惩罚项是$L^1$的, 有变量选择的效果.
L1约束的菱形角点容易与最小二乘等高线相交于坐标轴上, 因此Lasso倾向于让部分系数精确为零.
